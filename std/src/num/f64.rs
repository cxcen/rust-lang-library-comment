//! `f64` 双精度（double-precision）浮点类型的相关常量。
//!
//! *[另见 `f64` 原始类型](primitive@f64)。*
//!
...
//! 直接定义在 `f64` 类型上的关联常量。
//!
//! 实现说明：本模块在 std 而非 core 中提供 `f64` 的浮点数学方法（如 `sqrt`、
...
impl f64 {
    ...
    /// 例如，`f64::powi(f64::NAN, 0)` 返回 `1.0`。但是，如果某个输入是一个 *signaling*（信号）
    ...
    ///
    /// 注意本函数较为特殊：对于 NaN 输入它也可能返回非 NaN 的结果。例如，
    /// 例如，`f64::powf(f64::NAN, 0.0)` 返回 `1.0`。但是，如果某个输入是一个 *signaling*（信号）
    /// NaN（即 *signaling* NaN），则结果不确定：要么是 NaN，要么是对应的安静（quiet）
    /// NaN 所产生的结果。
    ///
    /// # 未指定精度(Unspecified precision）
