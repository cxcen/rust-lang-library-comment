//! 扩展精度表示使用的 5 的幂预计算表。
//!
//! 这些表让算法能在 128 位或 192 位表示中，根据十进制 exponent 快速缩放浮点有效数字，
//! 并把中间舍入误差控制在可判定范围内。
//!
//! DO NOT MODIFY: 由 `src/etc/dec2flt_table.py` 生成。
...
// 使用 static 以避免编译时间过长：Rust 编译器错误可能让整张表被编译多次，
// 并多次生成代码，即使最终二进制会把它剔除。
#[rustfmt::skip]
