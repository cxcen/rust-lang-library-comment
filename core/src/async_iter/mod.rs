...
//! [`Waker`](crate::task::Waker) 之上——`poll_next` 返回 `Poll<Option<Item>>`,其中
//! `Ready(None)` 表示流终止;真正驱动它前进、在元素就绪时唤醒任务的是上层运行时。
//!
...
//!   方法,而不是 `struct` 本身。至于原因,详见 '[实现 Async Iterator](#implementing-async-iterator)'。
//!
//! [Traits]: #traits
//!
//! 就这些!下面我们深入异步迭代器。
...
//! 产出为止。
//!
//! 个别异步迭代器可能会选择恢复迭代,因此再次调用 `next` 可能会(也可能不会)在某个时刻再次产出
//! `Some(Item)`。
//!
...
//! 之前,实际上什么都不会发生。当有人仅仅为了副作用而创建异步迭代器时,这有时会成为困惑的来源。
//! 编译器会就这类行为向我们发出警告:
//!
//! ```text
